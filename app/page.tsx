...
type Types = "todos" | "aguardando" | "calculos recebidos" | "enviado/outros";
...
export default function Home() {
  ...
  // Filtrar os formulários baseado no status selecionado e termo de busca
  const filteredForms = forms.filter((form) => {
    // Filtro por status
    const statusMatch = 
      selected === "todos" || 
      (selected === "aguardando" && form.statusFirebase === "aguardando") ||
      (selected === "calculos recebidos" && form.statusFirebase === "lucro calculado") ||
      (selected === "enviado/outros" && (form.statusFirebase === "enviado"));
    
    // Filtro por busca (busca em múltiplos campos)
    ...
    const searchMatch = 
      ...
      form.codigo?.toLowerCase().includes(searchLower) ||
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
          <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
                  selected === "calculos recebidos"
                    ? "bg-slate-900 text-white"
                    : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                }`}
                onClick={() => setSelected("calculos recebidos")}
              >
                Cálculos Recebidos
              </button>
              <button
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
                  selected === "enviado/outros"
                    ? "bg-slate-900 text-white"
                    : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                }`}
                onClick={() => setSelected("enviado/outros")}
              >
                Enviado/Outros
              </button>
            </div>
                  ...
                  filteredForms.map((form) => (
                    <tr key={form.idFirebase} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                        {form.data ? new Date(form.data).toLocaleDateString('pt-BR') : '-'}
                      </td>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap font-bold">
                        R$ {form.precoVenda?.toFixed(2) || '0,00'}
                      </td>
                      <td className="px-4 py-3 text-sm whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          form.status === 'fechado' 
