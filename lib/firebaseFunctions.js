...
async function sendForm(formData, role, empresa) {
    ...
    updates[`${empresa}/calculos/geral/${newId}`] = {
        ...
        observacoesSecundarias: formData.observacoesSecundarias || '',
...
async function getFormById(idFirebase, role, empresa, userUid) {
    try {
        if (!idFirebase || !empresa) return null;
        if (role !== "admin" && !userUid) return null;
        ...
        if (role === "admin") {
            const [geralSnapshot, lucroSnapshot] = await Promise.all([
                ...
                get(ref(db, `${empresa}/calculos/lucro/${idFirebase}`)),
            ]);
            geral = geralSnapshot.val();
            lucro = lucroSnapshot.val();
        } else {
            const geralRef = ref(db, `${empresa}/calculos/geral`);
            ...
            const listaQuery = query(
                geralRef,
                orderByChild("statusFirebase"),
                equalTo("lista")
            );
            const [snapResponsavel, snapLista] = await Promise.all([
                get(responsavelQuery),
                get(listaQuery),
            ]);
            geral =
                snapResponsavel.val()?.[idFirebase] ??
                snapLista.val()?.[idFirebase] ??
                null;
        }
        ...
        const formData = {
            ...
            observacoesSecundarias: geral.observacoesSecundarias || '',
            validade: geral.validade || '',
        };
...
async function getAllForms(role, userUid, empresa) {
    try {
        if (role === "admin") {
            ...
            Object.entries(geral).forEach(([firebaseId, form]) => {
                let formadd = {
                    ...
                    observacoesSecundarias: form.observacoesSecundarias || '',
        ...
        const geralPorResponsavel = snapResponsavel.val() || {};
        const geralPorLista = snapLista.val() || {};
        const merged = { ...geralPorResponsavel };
        Object.entries(geralPorLista).forEach(([firebaseId, form]) => {
            if (!merged[firebaseId]) merged[firebaseId] = form;
        });

        const allForms = [];
        Object.entries(merged).forEach(([firebaseId, form]) => {
            allForms.push({
                idFirebase: firebaseId,
                id: form.id,
                status: form.status,
...
async function uploadFile(empresa, { file: file, path: path, idFirebase: idFirebase }) {
    try {
        const fileRef = storageRef(storage, `${empresa}/${path}/${idFirebase && idFirebase !== '' ? idFirebase + '/' : ''}${file.name}`);
...
async function registrarProdutoEstoque(empresa, { produto: produto, marca: marca, modelo: modelo, quantidade: quantidade, observacoes: observacoes, foto: fotoUrl }) {
    try {
        const newId = push(ref(db, `${empresa}/estoque/produtos`)).key;
        const updates = {};
        updates[`${empresa}/estoque/produtos/${newId}`] = {
            produto: produto,
            marca: marca,
            modelo: modelo,
...
async function atualizarProdutoEstoque(empresa, { idFirebase: idFirebase, produto: produto, marca: marca, modelo: modelo, quantidade: quantidade, observacoes: observacoes, foto: fotoUrl }) {
    try {
        update(ref(db, `${empresa}/estoque/produtos/${idFirebase}`), {
            produto: produto,
            marca: marca,
            modelo: modelo,
            quantidade: quantidade,
...
async function atribuirPrecoProdutoEstoque(empresa, { idEstoque: idEstoque, precoVenda: precoVenda, tipoFrete: tipoFrete }) {
    try {
        update(ref(db, `${empresa}/estoque/produtos/${idEstoque}`), {
        precoVenda: precoVenda,
        tipoFrete: tipoFrete,
    });
        return { success: true };
    } catch (error) {
        console.error('Erro ao atribuir preço ao produto:', error);
        throw error;
    }
}
