...
// set tickets comissoes
async function setTicketsComissoes(empresa, ticketsList) {
    try {
        const batch = writeBatch(db);

        const comissoesRef = doc(collection(db, `${empresa}/comissoes`));
        batch.set(comissoesRef, ticketsList);
...
async function setComissao(empresa, ticketsList) {
